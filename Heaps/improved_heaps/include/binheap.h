 ...
 *
 *  This class is meant to represent binary heaps.
 */
typedef struct {
    void *A;                  // used to store heap nodes
    unsigned int *key_pos;    // used to store the position of the keys of a given node
    unsigned int *rev_pos;    // used to store the node corresponding to given positions in A
