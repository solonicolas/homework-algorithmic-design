...
{ 
    ...
    float ***S = (float ***)malloc(sizeof(float **) * 10); // array of matrices

    for (size_t i = 0; i < 10; ++i) {
        S[i] = allocate_matrix(n2, n2);
    }

    float ***P = (float ***)malloc(sizeof(float **) * 7); // array of matrices
                  ...
                  const size_t n)
{
    ...
    // P4 = A22 x S4
    strassen_aux_improved(P[2], A, (const float* const *const) S[0],
                 0, 0,
                 A_f_row + n2, A_f_col + n2,
                 0, 0,
                 n2);
    ...
    // P5 = S5 x S6
    strassen_aux_improved(P[3], (const float* const *const) S[0],
    ...
    // C11 = P5 + P4 - P2 + P6
    sum_matrix_blocks(C, (const float* const *const) P[3],
                      (const float* const *const) P[2],
                      C_f_row, C_f_col,
                      0, 0,
                      0, 0,
    ...
    // C21 = P3 + P4
    sum_matrix_blocks(C, (const float* const *const) P[1],
                      (const float* const *const) P[2],
                      C_f_row+n2, C_f_col,
                      0, 0,
                      0, 0,
...
void strassen_matrix_multiplication_improved(float **C, float const *const *const A,
                                    float const *const *const B, size_t n)
{
  strassen_aux_improved(C, A, B, 0, 0, 0, 0, 0, 0, n);
}